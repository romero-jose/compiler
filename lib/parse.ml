...
let parse_literal (s : string) : expr =
  ...
  | _ -> (
      ...
      | None ->
          ...
          else Fmt.failwith "parse: %s is not a valid identifier" s)
...
let rec parse (sexp : sexp) : expr =
  ...
  | `List [ `Atom "let"; `List [ `Atom id; binding ]; s ] ->
      ...
      else Fmt.failwith "parse: %s is not a valid identifier" id
  ...
  | _ -> Fmt.failwith "parse: Invalid symbolic expression %a" pp sexp

let parse_string : string -> expr =
 fun s ->
  match CCSexp.parse_string s with
  | Ok sexpr -> parse sexpr
  | Error error -> Fmt.failwith "parse: Can't parse string %s" error
